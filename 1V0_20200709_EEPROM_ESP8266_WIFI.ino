...
#define WIFIPORT 8023
/* The device connects to a hotspot, and on this hotspot, it attempts to */
/* connect to a port. On this port, you can listen to it - for instance */
/* through netcat, e.g. with "nc -l -p 8023 -k" - netcat acts as server. */
/* This makes it easy to connect it to your mobile phone's hotspot. */
...
#define DATAMEM 1801
/* I.e. x4 using 7204 bytes of RAM */
#define EXECMEM 1001
...
unsigned long returnreadunsignedlong = 0;
...
long readlong() {
  for (m = 0; m < NRSIZE; m++) { yield();
    numbufr[m] = 0; /* sanitize the numeric buffer */
  }

  m = 0;
  while (m < NRSIZE) { yield();
    if (client.available()) {
      asciidigit = static_cast<char>(client.read());
      if ((asciidigit != 0) &&
          (asciidigit != '\r') &&
          (asciidigit != '\n') &&
          (asciidigit != '\t') &&
          (asciidigit != ' ')) {
        numbufr[m] = asciidigit;
      } else {
        break;
      }
      m++;
...
//  returnreadlong = (static_cast<long>(atol(numbufr)));
  sscanf(numbufr, "%ld", &returnreadlong);
...
unsigned long readunsignedlong() {
  ...
  sscanf(numbufr, "%lu", &returnreadunsignedlong);
  return(returnreadunsignedlong);
}
...
//  returnreaddouble = (static_cast<double>(atof(numbufr)));
  sscanf(numbufr, "%lf", &returnreaddouble);
...
delay(1000);
int flg = 0;
if  (!client.connect(WiFi.gatewayIP(), WIFIPORT)) { return;  } else { flg = 1;}

while ((cmnt = client.read()) != '\n' && cmnt != EOF ) { yield(); } /* Press Enter to start */
// client.print(F("\r\n"));; // Sometimes it prints something BEFORE it is ready -
                  // let it be at least something irrelevant.

/* Initialise commands to zero, which is no-op */

/*
for (i=0; i<EXECMEM; i++) { yield();
...
if (cmdadr >= 0) {
  ...
  client.print(F("OPERATION: "));
  opr = readlong();
  client.print(F("DATA ADR1: "));
  datadr1 = readlong();
  client.print(F("DATA ADR2: "));
  datadr2 = readlong();
/* COMMENT OUT, IF DESIRED, FROM HERE ...: */
  ...
  client.print(F("CMNT: ")); /* A comment is not saved, merely printed to the */
...
/* ... TO HERE. */
  ...
  if ((datadr1 >= DATAMEM) || (datadr2 >= DATAMEM) ||
    ...
    client.print(F("CORR RANGE. DATA 0 ... "));
    client.print((DATAMEM - 1));
    client.print(F(", INSTR 0 ... "));
    client.print((EXECMEM - 1));
    client.print(F("\r\n"));;
  }
  ...
  if (cmdadr < 1000) { client.print(F("0")); } /* "leading zeroes" */
  if (cmdadr < 100) { client.print(F("0")); }
  if (cmdadr < 10) { client.print(F("0")); }
  client.print(cmdadr);
  client.print(F(" "));
  opr = (instr >> 26) & 63; //-----------------------------------------------------------------------------------------------------------------
  printopr();
  ...
  if (datadr1 < 1000) { client.print(F("0")); } /* "leading zeroes" */
  if (datadr1 < 100) { client.print(F("0")); }
  if (datadr1 < 10) { client.print(F("0")); }
  datadr1 = (instr >> 13) & 8191; //-------------------------------------------------------------------------------------------------------------
  client.print(datadr1);
  client.print(F(">"));
  client.print(static_cast<double>(datum[datadr1]), 8); 
  ...
  if ((datum[datadr1] > -1 * DATAMEM) && (datum[datadr1] < DATAMEM)) {
    ...
    client.print(F(">"));
    client.print(static_cast<double>(datum[j]), 8); 
    /* scientific, big E, sign */
    /* 4: E, not e, 1: plus or space, 2: of these plus and not space. */
  }
  client.print(F(" "));

  if (datadr2 < 1000) { client.print(F("0")); } /* "leading zeroes" */
  if (datadr2 < 100) { client.print(F("0")); }
  if (datadr2 < 10) { client.print(F("0")); }
  datadr2 = instr & 8191; // -------------------------------------------------------------------------------------------------------------------
  client.print(datadr2);
  client.print(F(">"));
  client.print(static_cast<double>(datum[datadr2]), 8); 
  /* scientific, big E, sign */
  /* 4: E, not e, 1: plus or space, 2: of these plus and not space. */
...
if (cmdadr == 0) {
  ...
  client.print(F("LIST INSTRUCTIONS"));
  client.print(F("\r\n"));
  client.print(F("1ST CMD ADR: "));
  datadr1 = readlong();
  /* atof and atoi due to lack of scanf */
  client.print(F("2ND CMD ADR: "));
  ...
  for (i = h; i <= k; i++) { yield();
    ...
    if (i < 1000) { client.print(F("0")); } /* "leading zeroes" */
    if (i < 100) { client.print(F("0")); }
    if (i < 10) { client.print(F("0")); }
    client.print(i);
    client.print(F(" "));
    if (opr < 10) { client.print(F("0")); }
    client.print(opr);
    client.print(F("="));
    printopr();
    ...
    client.print(F("DECIMAL REPRESENTATION: "));
    client.print(instr);
    client.print(F("\r\n"));
  ...
  client.print(F("LIST DATA"));
  client.print(F("\r\n"));
  client.print(F("1ST ADR: "));
  datadr1 = readlong();
  client.print(F("2ND ADR: "));
  datadr2 = readlong();
  if ((datadr1 < 0) || (datadr2 < 0)) { break; } /* fast eject */
  ...
  for (i = h; i <= k; i = i + 2) { /* Print as many as the terminal allows. */
    ...
    client.print(i);
    client.print(F(" "));
    client.print(static_cast<double>(datum[i]), 8); 
    /* scientific, big E, sign */
    /* 4: E, not e, 1: plus or space, 2: of these plus and not space. */
    client.print(F(" "));
    client.print(static_cast<double>(datum[i+1]), 8);
    client.print(F("\r\n"));
  }
  ...
  client.print(F("ENTER FULL INSTRUCTIONS AS DECIMALS, 67108863 TO END"));
  client.print(F("\r\n"));
  client.print(F("START ADR: "));
  cmdadr = readlong(); /* From here upwards you enter instructions. */
  if (cmdadr < 0) { break; } /* fast eject */
  while (cmdadr < EXECMEM) {
    yield();
    if (pc < 1000) { client.print(F("0")); } /* "Leading zeroes. */
    if (pc < 100) { client.print(F("0")); }
    if (pc < 10) { client.print(F("0")); }
    client.print(cmdadr);
    client.print(": INSTRUCTION: ");
    instr = readunsignedlong();

    if (instr == 67108863) { /* eject at NOP 8191 8191 */
      break;
    } else {
      EEPROM.put((INSTRUCTION_BYTE_LENGTH * (cmdadr - 0)) + ZERO_OFFSET, instr);
      // EEPROM.commit();
  ...
  client.print(F("ENTER DATA FLOATS"));
  client.print(F("\r\n"));
  client.print(F("1ST ADR: "));
  datadr1 = readlong();
  client.print(F("2ND ADR: "));
  ...
  while (datadr1 <= datadr2) {
    yield();
    if (datadr1 < 1000) { client.print(F("0")); } /* "Formatting with leading zeroes". */
    if (datadr1 < 100) { client.print(F("0")); }
    if (datadr1 < 10) { client.print(F("0")); }
    client.print(datadr1);
    client.print(F(": DATUM: "));

    f = readdouble();
    client.print(static_cast<double>(f), 8); 
    client.print(F("\r\n"));;
    client.flush();
  ...
  client.print(F("CLEAR INSTRUCTION RANGE"));
  client.print(F("\r\n"));
  client.print(F("1ST CMD ADR: "));
  datadr1 = readlong();
  client.print(F("2ND CMD ADR: "));
  ...
  client.print(F("INSTRUCTIONS CLEARED"));
  client.print(F("\r\n"));
  ...
  client.print(F("CLEAR DATA RANGE"));
  client.print(F("\r\n"));
  client.print(F("1ST ADR: "));
  datadr1 = readlong();
  client.print(F("2ND ADR: "));
  ...
  /* as the program runs. */
  client.print(F("TRACE ON"));
  client.print(F("\r\n"));
  tracer = 1;
} else if (cmdadr == -8) {
  client.print(F("TRACE OFF"));
  client.print(F("\r\n"));
  tracer = 0;
} else if (cmdadr == -9) {
  client.print(F("SET RUNLIMIT, UP TO 32767, 0=INFINITE, TERMINATING WHEN 1: "));
  runlimit = readlong();
} else if (cmdadr == -10) {
...
client.print("Mid-air: instr=");
client.print(instr);
client.print(" EEPROMLONG[0]=");
...
client.print(" zz=");
client.print(zz);
client.print(" oprBefore=");
client.print(opr);
client.print(" d1Before=");
client.print(datadr1);
client.print(" d2Before=");
client.print(datadr2);
client.print(F("\r\n"));;
...
/* EEPROM committed in mid-air - balance between "wear" and "safety" */
EEPROM.commit();
...
while (pc < EXECMEM) {
  yield();
  if (runlimit == 1) {
    client.print(F("RUN EXHAUSTED PC="));
    client.print(pc);
    client.print(F("STOPPED"));
    client.print(F("\r\n"));;
    break; /* ejection if the runlimit is over, pc printed for re-entry */
  } else if (runlimit > 1) {
    runlimit--;
    if (((runlimit - 1) % 100) == 0) {
      client.print(F("RUNLIMIT = "));
      client.print(runlimit);
      client.print(F("\r\n"));;
  ...
  /* Arduino should get delays here, due to printing. */
    ...
    client.print(pc);
    client.print(F(" "));
    // if (cmdadr < 100) { client.print(F("0")); } /* if >99 instr. */
    if (cmdadr < 10) { client.print(F("0")); }
    client.print(opr);
    client.print(F("="));
    printopr();
...
/*   client.print("Entering decoder.");
  ...
  /* that only the first address is to be printed. */
    ...
    if ((datadr1 == datadr2) || (datadr2 == 0)) {
      ...
      for (i = datadr1; i <= datadr2; i++) { yield();
        f = datum[i];
        client.print(F("OUTPUT: ADR="));
        if (i < 1000) { client.print(F("0")); } /* "leading zeroes" */
        if (i < 100) { client.print(F("0")); }
      ...
      for (i = datadr2; i <= datadr1; i++) { yield();
        f = datum[i];
        client.print(F("OUTPUT: ADR="));
        if (i < 1000) { client.print(F("0")); } /* "leading zeroes" */
        if (i < 100) { client.print(F("0")); }
  ...
  /* indicating that only a single number is to be read. */
    ...
    client.print(F("DATA INPUT INTO RANGE "));
    client.print(datadr1);
    client.print(F(" TO "));
    client.print(datadr2);
    client.print(F("\r\n"));;
    if ((datadr1 == datadr2) || (datadr2 == 0)) {
      client.print(F("INPUT:  ADR="));
      client.print(datadr1);
      client.print(F(" DATUM="));
  ...
  /* datadr1 holds the initial value and datadr2 holds the step. */
    ...
    if ((datadr1 == datadr2) || (datadr2 == 0)) {
      ...
      f = datum[datadr2];
      for (i = datadr1 + 1; i <= datadr2; i++) { yield();
  ...
  /* actually care about the largest and smallest absolute values. */
    ...
    if (h < k) {
      for (i = h + 1; i <= k; i++) { yield(); /* after the first number */
        if (f < datum[i]) {
          f = datum[i];
        }
        if (g > datum[i]) {
      ...
      for (i = k; i < h; i++) { yield(); /* before the last number */
        if (f < datum[i]) {
          f = datum[i];
        }
        if (g > datum[i]) {
  ...
  /* datadr2, both inclusive. */
    ...
    if ((datadr1 == datadr2) || (datadr2 == 0)) {
      ...
      for (i = datadr1; i <= (datadr1 + ((datadr2 - datadr1) / 2)); i++) { yield();
        f = datum[i];
        datum[i] = datum[datadr2 - (i - datadr1)];
        datum[datadr2 - (i - datadr1)] = f;
      }
    } else if (datadr1 > datadr2) {
      for (i = datadr2; i <= (datadr2 + ((datadr1 - datadr2) / 2)); i++) { yield();
        f = datum[i];
  ...
  /* of the shorter range. */
    ...
    if ((h == datadr1) || (k == datadr2)) {
      ...
      j = datadr2 + 1;
      for (i = datadr1 + 1; ((i <= h) && (j <= k)); i++) { yield();
