...
/* Note to self - how to catch "=" instead of "==" in an if-condition:      */
/* cat file.c | grep if | grep " = " ... will show most dangerous lines.    */

#include <stdio.h>
#include <stdlib.h>
#include <math.h>

/* These DATAMEM and EXECMEM are set to mimic a MEGA2560: */
#define DATAMEM 2001
#define EXECMEM 1001
...
while (1) {
pc = 0; /* safety: nuke the program counter */
if (instruction[0] != 0) {
  instruction[0] = 0;   /* safety: erase the reserved instruction */
} /* The theatre with the "if" is in case this becomes EEPROM.
...
/* COMMENT OUT, IF DESIRED, FROM HERE ...: */
  ...
  printf("CMNT (OPT) : "); /* A comment is not saved, merely printed to the */
...
/* ... TO HERE. */
  ...
  if ((datadr1 >= DATAMEM) || (datadr2 >= DATAMEM) ||
    ...
    printf("CORRECTING RANGE. DATA 0 ... %d, INSTRUCTIONS 0 ... %d.\n",
          (DATAMEM - 1), (EXECMEM - 1));
  }
  ...
  /* This assumes an operand of 8 bit and two 12 bit data addresses each. */
  ...
  for (i = h; i <= k; i++) {
    instr = instruction[i];
    opr = (instr >> 26) & 63; /* 4095:255:24:12 or 8191:63:26:13 */
    datadr1 = (instr >> 13) & 8191;
    datadr2 = instr & 8191;
    printf("%05d %02d=", i, opr); /* turn %02d into %03d if >99 instr. */
    if (opr == 0) {
      printf("NOOP");
    } else if (opr == 1) {
      printf("JUMP");
    ...
    printf("%u", instr); /* I did NOT make %09u, so this is not */
                         /* misunderstood as octal if read elsewhere. */
    printf("\n");
  ...
  for (i = h; i <= k; i = i + 5) { /* Print as many as the terminal allows. */
    printf("%04d: %+E %+E %+E %+E %+E\n",
      i, datum[i], datum[i+1], datum[i+2], datum[i+3], datum[i + 4]);
  }
  ...
  printf("ENTER FULL INSTRUCTIONS AS DECIMAL NUMBERS, "); 
  printf("67108863 TO TERMINATE.\n");
  printf("START ADDRESS FROM WHERE ON TO ENTER: ");
  scanf("%u", &cmdadr); /* From here upwards you enter instructions. */
  if (cmdadr < 0) { break; } /* fast eject */
  while (cmdadr < EXECMEM) {
    printf("%05d: INSTRUCTION: ", cmdadr);
    scanf("%u", &instr);
    if (instr == 67108863) { /* again, eject */
      break;
    } else {
      instruction[cmdadr] = instr; /* no interpretation, just acceptance */
    }
    cmdadr++;
  }
  ...
  printf("ENTER DATA AS FLOATING POINT NUMBERS ");
  printf("WITHIN INCLUSIVE RANGE LIMITS.\n");
...
while (pc < EXECMEM) {
  ...
  /* Arduino will have delays here, due to printing. */
    printf("%05d %02d=", pc, opr);  /* turn %02d into %03d if >99 instr. */
    if (opr == 0) {
      printf("NOOP");
    } else if (opr == 1) {
      printf("JUMP");
  ...
  /* that only the first address is to be printed. */
    ...
    printf("DATA OUTPUT OF THE RANGE %04d TO %04d:\n", datadr1, datadr2);
    if ((datadr1 == datadr2) || (datadr2 == 0)) {
      f = datum[datadr1];
      printf("OUTPUT: ADDRESS=%04d DATUM=%+E\n", datadr1, f);
  ...
  /* indicating that only a single number is to be read. */
    ...
    printf("DATA INPUT INTO THE RANGE %d TO %d:\n", datadr1, datadr2);
    if ((datadr1 == datadr2) || (datadr2 == 0)) {
      printf("INPUT:  ADDRESS=%04d DATUM=", datadr1);
      scanf("%f", &f);
      datum[datadr1] = f;
    } else if (datadr1 < datadr2) {
      for (i = datadr1; i <= datadr2; i++) {
        printf("INPUT:  ADDRESS=%04d DATUM=", i);
        scanf("%f", &f);
        datum[i] = f;
      }
    } else if (datadr1 > datadr2) {
  ...
  /* sorting the first range datum[datadr1 + 1] ... datum[datum[datadr1]]. */
    ...
    h = abs(round(datum[datadr1])); /* Only datum[datadr1] determines range. */
